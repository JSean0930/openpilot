...
class CarController(CarControllerBase):
  def __init__(self, dbc_names, CP):
    ...
    # Corolla TSS2：方向盤回正時常需要更高的角速度，避免『彎已結束但還不回正』
    # 保守加一點上限，仍保留 >100 deg/s fault prevention 的機制。
    self.max_steer_rate = 120 if CP.carFingerprint == CAR.TOYOTA_COROLLA_TSS2 else MAX_STEER_RATE
  ...
  def update(self, CC, CS, now_nanos):
    ...
    self.steer_rate_counter, apply_steer_req = common_fault_avoidance(abs(CS.out.steeringRateDeg) >= self.max_steer_rate, lat_active,
