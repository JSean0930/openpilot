...
class CarControllerParams:
  ...
  def __init__(self, CP):
    ...
    # --- Corolla TSS2 (2020 Corolla Hatchback 等) 方向盤回正/扭力權限加強 ---
    # 目的：
    # - 提升低速/市區彎道的可用扭力與扭力變化速度（更願意打、也更快回正）
    # - 數值以保守增量起手，避免 EPS fault；若仍覺得不夠可再微調
    if CP.carFingerprint == CAR.TOYOTA_COROLLA_TSS2:
      # 提升最大扭力上限（原 1500）。太高可能觸發 EPS/PCS fault，建議 1650~1750 範圍內測試。
      self.STEER_MAX = 1700
      # 扭力命令變化速度：UP 決定「打進去」速度；DOWN 決定「放回來」速度（回正感）
      if CP.lateralTuning.which() == 'torque':
        self.STEER_DELTA_UP = 18
        self.STEER_DELTA_DOWN = 32
      else:
        self.STEER_DELTA_UP = 12
        self.STEER_DELTA_DOWN = 32
      # 放寬 cmd 與 EPS 量測扭力允許誤差（避免因噪聲導致過早削扭）
