...
SPEED_RATIO_OFF_HIGH = 0.955  # 🌟 新增：高速退出門檻，容許掉更多速度才補油，延長滑行
...
# ----------------------------------------------------
IRON_DIST_MAX = 3.0        # 距離 > 3.0m 開始準備啟動熨斗
...
class ACM:
  ...
  def _should_activate(self, user_ctrl_lon, v_ego, v_cruise, current_time):
    ...
    # 🌟 優化：啟動門檻平滑化 (無縫銜接低速與高速的滑行觸發點)
    ratio_on = smooth_interp(v_kph, [MIN_ACM_SPEED_KPH, HIGH_SPEED_KPH], [SPEED_RATIO_ON, SPEED_RATIO_ON_HIGH])
  ...
  def _should_deactivate(self, v_ego, v_cruise, current_time):
    ...
    # 🌟 優化：退出門檻平滑化 (高速時容許掉更多速度，大幅延長巡航滑行時間)
    ratio_off = smooth_interp(v_kph, [MIN_ACM_SPEED_KPH, HIGH_SPEED_KPH], [SPEED_RATIO_OFF, SPEED_RATIO_OFF_HIGH])
    
    if v_ego < (v_cruise * ratio_off) or self._has_lead: return True
    return False
