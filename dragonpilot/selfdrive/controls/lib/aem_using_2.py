...
# AEM v2（精簡版）
# 需求：
# - 時速 < 50 km/h：不啟動 AEM（完全不覆蓋 blended，也不累積觸發狀態）
# - 時速 >= 50 km/h：才允許 AEM 依 stopline 判斷切換到 blended
# - 移除你原本 A) 5~30kph 強制鎖定相關功能與用不到的代碼
# ============================================================
...
class AEM:
  ...
  def get_mode(self, mode, v_ego=None):
    """
    - v < 50 km/h：不啟動 AEM，直接回傳原 mode
    - v >= 50 km/h：允許 AEM 覆蓋 blended（硬覆蓋 -> 權重覆蓋）
    """
    t_now = time.monotonic()

    # 50km/h 以下：完全禁用 AEM（也清狀態避免殘留）
    if v_ego is not None:
      v_kph = v_ego * 3.6
      if v_kph < AEM_ENABLE_MIN_KPH:
        if self._active:
  ...
  def update_states(self, model_msg, radar_msg, v_ego):
    """
    - v < 50 km/h：不啟動 AEM（不累積任何觸發狀態）
    - v >= 50 km/h：檢查 stopline 判停條件，滿足後觸發 experimental
    """
    v_ego_kph = v_ego * 3.6

    # 50km/h 以下：完全禁用 AEM（也清狀態避免殘留）
    if v_ego_kph < AEM_ENABLE_MIN_KPH:
      if self._active:
        self._reset_state_immediate()
      return
