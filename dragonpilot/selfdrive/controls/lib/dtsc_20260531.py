...
CURV_LPF_ALPHA = 0.20         # 曲率 horizon 低通濾波 0.35 (入彎時使用)
LOG_EVERY_N = 20              # log 節流

MIN_ENTRY_DECEL = -0.15       # 觸發後最小入彎減速（作為 trail braking 的保底維持力道）-0.25
...
class DTSC:
  ...
  def get_mpc_constraints(self, model_msg, v_ego, base_a_min, base_a_max, v_set_mps=None):
    ...
    # ==========================================================
    # ✅ 擬人化煞車動態控制：S 曲線起煞 + Trail Braking 帶煞入彎
    # ==========================================================
    
    # 計算煞車加深期 (Brake Build-up) 的時間，受限於 RAMP_MIN_S 與 RAMP_MAX_S
    ramp_time = float(np.clip(t_total * 0.6, RAMP_MIN_S, RAMP_MAX_S))
    t_peak = t_start + ramp_time

    # 確保釋放期 (Trail Braking) 至少有 0.2 秒的空間
    t_crit = max(t_crit, t_peak + 0.2)
    
    # 為了補償 S 曲線與釋放期造成的平均煞車力下降，稍微放大目標峰值 (交給 MPC 閉環微調)
    ...
    for i, t in enumerate(T_IDXS_MPC):
      ...
      # ✅ 優化：動態出彎解封邏輯
      # 如果預測時間已過彎心，且前方該點的預測曲率已低於判定門檻（路直了），立刻解除約束，解放油門！
      if t > t_crit and curv_s[i] < MIN_CURVATURE:
        break
        
      # 安全保底：過了彎心 0.5 秒後也自動完全撤銷限制，絕不拖泥帶車
      if t > t_crit + 0.5:
        break

      if t <= t_peak:
        # 【階段一：Smoothstep 平滑起煞】
        # 消除起煞撞牆感，從 0 柔和地過渡到 peak_decel
        p = np.clip((t - t_start) / max(ramp_time, 1e-3), 0.0, 1.0)
        p_smooth = p * p * (3.0 - 2.0 * p) 
        a_cap = p_smooth * peak_decel
        
      elif t <= t_crit:
        # 【階段二：帶煞入彎 Trail Braking】
        # 接近彎心時，從 peak_decel 慢慢釋放回 MIN_ENTRY_DECEL (-0.15)
        p = np.clip((t - t_peak) / max(t_crit - t_peak, 1e-3), 0.0, 1.0)
        p_smooth = p * p * (3.0 - 2.0 * p)
        
        # 確保極輕微減速時，不會在 Trail Braking 階段反而加重煞車
        target_trail_decel = max(peak_decel, MIN_ENTRY_DECEL)
        a_cap = peak_decel + p_smooth * (target_trail_decel - peak_decel)
        
      else:
        # 【階段三：彎心動態解封與加速準備】
        # ✅ 優化：快速（0.4秒內）將約束從小減速平滑拉高到原車最大加速度 base_a_max，而非死卡在 0 附近。
        # 這樣能讓 MPC 提前看到前方「被允許重力加速」，進而提早命令 TSS2 電子節氣門建立增壓與推力。
    ...
    self.debug_msg = (
      ...
      f"req {req_decel:.2f}, peak {peak_decel:.2f}, t_start {t_start:.2f}, t_crit {t_crit:.2f}"
    )
    self._log_cnt += 1
