...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp_cam):
    ...
    if self.CP.carFingerprint in UNSUPPORTED_DSU_CAR or self.CP.carFingerprint in RADAR_ACC_CAR:
      ret.distanceLines = 2
    ...
    # if self.CP.carFingerprint in RADAR_ACC_CAR and self.CP.carFingerprint != CAR.CHR_TSS2:
    ...
    ret.adjustableFollowCar = True if (self.CP.carFingerprint in (TSS2_CAR - RADAR_ACC_CAR) or (self.CP.flags & ToyotaFlags.SMART_DSU)) else False
    ret.steeringWheelCar = ret.adjustableFollowCar
