...
# 取代原有的單一常數 ACCEL_CLIP_SLEW_NORMAL / FAST
# ============================================================
# 車速節點 (m/s) [0, 40km/h, 80km/h, 120km/h]
SLEW_V_BP = [0., 11.1, 22.2, 33.3] 

# 加速變化率上限 (m/s^3)：低速起步允許快(2.5)，高速巡航極度收斂(0.6) 避免推背感
ACCEL_SLEW_RATE_BP = [2.5, 1.4, 0.9, 0.6] 

# 減速變化率下限 (m/s^3)：全速域保持敏捷，低速(2.5)應對塞車，高速(1.5)平穩降速
# 這裡存正值，因為下方 _accel_clip_slew_step 是當作 delta_down (變化量) 來用
...
def _accel_clip_slew_step(dt: float, v_ego: float, lead_a: float, trigger_approach: bool, ttc: float, a_req: float) -> tuple[float, float]:
  ...
  if fast:
    # 當遇到前方急煞或高危險狀態時，進一步放寬下踩煞車的變化率極限 (最大可到 3.0)
    base_slew_down = max(base_slew_down, 3.0)

  delta_up = float(max(0.01, base_slew_up * dt))       # 最低給 0.01 避免完全卡死
  delta_down = float(max(0.01, base_slew_down * dt))   # 最低給 0.01 避免完全卡死
