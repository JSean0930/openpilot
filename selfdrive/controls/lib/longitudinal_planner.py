...
# ============================================================
# ✅ 老司機平滑曲線算法 (Cosine Interpolation)
# 取代生硬的 np.interp，將節點間的直線轉化為無段的 S 型平滑曲線，消除頓挫
# ============================================================
...
class LongitudinalPlanner:
  ...
  def update(self, sm, dp_flags=0):
    ...
    # 時速 >= 40.0 km/h 時，徹底封印 e2e 模型的幽靈煞車，回歸穩如泰山的純雷達 MPC
    # ============================================================
    ...
    if _lead is not None and np.isfinite(_d_rel):
      ...
      if v_ego < 3.0 and _v_lead < 1.5 and _d_rel < 8.0:
        # 動態上限：前車越慢，油門上限越小；前車一旦加速，上限跟著放寬 (最低保障 0.2 m/s^2)
