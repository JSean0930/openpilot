...
class LongitudinalPlanner:
  ...
  def update(self, sm, dp_flags=0):
    ...
    # 2. 解除封印：高精度紅綠燈與路口判斷 (跨越 25km/h 限制)
    if not has_lead and (v_ego * CV.MS_TO_KPH < 70.0):
      # 讀取當前影格視覺模型的煞停意圖
      current_is_stopping = bool(sm['modelV2'].action.shouldStop) or (e2e_a < -0.4)
      
      # 🛡️ 機制一：視覺記憶暫留 (Vision Latch Timer)
      # 解決「閃爍問題」。只要模型看到一次紅燈，我們強制給予 0.6 秒的記憶鎖定期，
      # 防止模型因為瞬間雜訊瞎掉而突然鬆開煞車。
      if current_is_stopping:
        if self.e2e_stop_timer <= 0.0:
          self.e2e_stop_a_ema = e2e_a # 首次發現紅燈，直接同步當前煞車力道
        self.e2e_stop_timer = 0.6     # 充滿 0.6 秒記憶
      else:
        self.e2e_stop_timer -= self.dt # 沒看到紅燈，記憶慢慢衰退

      # 進入紅綠燈記憶煞停狀態！
      if self.e2e_stop_timer > 0.0:
        self.output_should_stop = True
        
        # 🛡️ 機制二：非對稱視覺濾波 (Asymmetric Vision Filter)
        # 解決「頓挫問題」。當模型想加深煞車時，我們光速跟上；當模型想放開煞車時，我們極度緩慢地放，確保平穩。
        if e2e_a < self.e2e_stop_a_ema:
          self.e2e_stop_a_ema = 0.1 * self.e2e_stop_a_ema + 0.9 * e2e_a  # 煞車加深極快
        else:
          self.e2e_stop_a_ema = 0.9 * self.e2e_stop_a_ema + 0.1 * e2e_a  # 放煞車極慢
          
        # 🛡️ 機制三：動態非線性煞車放大 (Dynamic Braking Amplifier)
        # 解決「無腦放大導致的不適感」與「煞車軟腳越線」的矛盾。
        # 輕煞車區間 (0 到 -0.5)：完全不放大 (乘 1.0)，保留極致平滑的滑行路感。
        # 重煞車區間 (-0.5 到 -1.5)：漸進式拉高放大係數，最高只放大 20% (乘 1.2)，確保煞停不越線。
        
        amp_ratio = smooth_interp(self.e2e_stop_a_ema, [-1.5, -0.5], [1.20, 1.0])
        amplified_e2e_a = self.e2e_stop_a_ema * amp_ratio
        
        # 最終選取：在 MPC 和 強化後的視覺煞車 中，取最深、最安全的力道
        base_a_target = min(mpc_a, amplified_e2e_a)
      else:
        self.e2e_stop_a_ema = 0.0 # 記憶歸零，徹底解除警報
