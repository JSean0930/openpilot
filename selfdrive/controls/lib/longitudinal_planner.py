...
class LongitudinalPlanner:
  ...
  def update(self, sm, dp_flags=0):
    ...
    # -------------------------------------------------------------
    # 4. 老司機情境分發 (互斥狀態機架構)
    # 依嚴重程度層層過濾：緊急防撞 > 煞停區段 > 主動追擊 > 安全滑行 > 巡航
    # -------------------------------------------------------------
    ...
    # =========================================================
      ...
      # 2. 如果此時 MPC 給的油門太保守 (還在發呆)，我們就強行介入
      if base_a_target < pursuit_accel:
        
        # 依據前車逃離的速度來決定介入權重 (跑越快，我們接管越多)
        w_pursuit = float(np.clip((abs(_closing) - 0.25) / 1.5, 0.0, 1.0))
        
        # 保命機制：如果距離實在壓得太近 (例如小於 4 米)，油門還是要收斂一下
    ...
    # [狀態五] 正常巡航 (Cruise) -> 沿用 final_a_target = base_a_target
