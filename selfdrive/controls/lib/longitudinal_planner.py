...
class LongitudinalPlanner:
  ...
  def update(self, sm, dp_flags = 0):
    ...
    if reset_state:
      ...
      # 🌟 重置自駕狀態時，同步同步濾波記憶，防止突波
      self.smooth_a_target = self.a_desired
    ...
    # ==========================================================
    # 🌟 OP 0.11.1 原生體感優化補丁 (極簡版)
    # ==========================================================
    
    # 1. 🚀 果斷起步補丁 (Decisive Takeoff)
    # 解決視覺模型在綠燈或低速跟車起步時反應遲鈍、被 min() 拖累的問題。
    lead = sm['radarState'].leadOne
    if mode == 'blended' and v_ego < 3.0 and lead.status:
      # 如果雷達確認前車已經開走 (車速 > 3.6km/h) 且距離拉開超過 4 公尺
      if lead.vLead > 1.0 and lead.dRel > 4.0:
        # 強制跳過 e2e 的猶豫，直接信賴傳統 MPC 算出的起步推力，並給予 0.5 m/s² 最低保底
        output_a_target = max(output_a_target_mpc, 0.5)

    # 2. 🧈 非對稱絲滑濾波 (Asymmetric Jitter Filter)
    # 解決端到端 (e2e) 每影格雜訊直接傳遞給變速箱造成的油門高頻微幅抖動。
    if output_a_target < self.smooth_a_target:
      # 煞車減速或收油門：保持高度敏捷 (0.2 比 0.8)，確保安全
      self.smooth_a_target = 0.2 * self.smooth_a_target + 0.8 * output_a_target
    else:
      # 踩油門加速：重度一階濾波 (0.95 比 0.05)，把所有視覺模型的微小鋸齒狀訊號徹底熨平
      self.smooth_a_target = 0.95 * self.smooth_a_target + 0.05 * output_a_target
      
    # 將平滑處理完畢的舒適目標加速度餵給硬體限制器
    output_a_target = self.smooth_a_target
    # ==========================================================
