...
class LongitudinalPlanner:
  ...
  def update(self, sm, dp_flags=0):
    ...
    # [狀態四] 安全跟隨滑行 (徹底消滅騎馬感：訊號熨斗升級版)
      ...
      # 距離大於 4 米、速差小於 2 m/s 就可以準備進入舒適區
      w_dist = float(np.clip((_d_rel - 4.0) / 4.0, 0.0, 1.0))
      w_close = float(np.clip((2.0 - abs(_closing)) / 1.5, 0.0, 1.0))
      
      # 取兩者的最小值作為基礎權重，並【乘上高速緩衝權重】
      # 這樣只要時速超過 70，這裡算出來必定是 0，後面的 EMA 濾波就會讓它平順歸零
      raw_coast_weight = min(w_dist, w_close) * w_speed_iron
