...
class LatControlTorque(LatControl):
  ...
  def update_limits(self):
    # PID 輸出限制用「可達側向加速度」來設，避免非線性 torque mapping 造成限制失真
    self.pid.set_limits(self.lateral_accel_from_torque(self.steer_max, self.torque_params),
                        self.lateral_accel_from_torque(-self.steer_max, self.torque_params))

  def _apply_torque_slew_limit(self, output_torque, v_ego):
  ...
  def update(self, active, CS, VM, params, steer_limited_by_safety, desired_curvature, curvature_limited, lat_delay):
    ...
    # --- 扭力斜率限制（柔順度核心，避免低速高 Kp + 摩擦補償導致抽動）---
    output_torque = float(self._apply_torque_slew_limit(output_torque, CS.vEgo))
