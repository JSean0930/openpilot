...
class LatControlTorque(LatControl):
  def __init__(self, CP, CI):
    ...
    if self.use_nn:
      ...
      self.past_future_len = (len(self.nn_future_times) + len(self.history_frame_offsets))
  ...
  def update(self, active, CS, VM, params, last_actuators, steer_limited, desired_curvature, desired_curvature_rate, llk, lat_plan=None, model_data=None):
    ...
    if not active:
      ...
      if self.use_nn and model_planner_good:
        ...
        nn_measurement_input = [CS.vEgo, measurement, 0.05 * actual_lateral_jerk, roll] \
        ...
        if lat_plan.curvatures[0] == self.curvature_0_last:
          ...
          nn_setpoint_input = [CS.vEgo, setpoint, 0.05 * desired_lateral_jerk, roll] \
                                + [setpoint] * self.past_future_len \
                                + past_rolls + future_rolls
          self.torque_from_setpoint = self.torque_from_nn(nn_setpoint_input)
          pid_log.error = self.torque_from_setpoint - torque_from_measurement

          # compute feedforward (same as nn setpoint output)
          nn_input = [CS.vEgo, desired_lateral_accel, setpoint - measurement, roll] \
