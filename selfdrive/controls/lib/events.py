...
class NoEntryAlert(Alert):
  def __init__(self, alert_text_2: str, visual_alert: car.CarControl.HUDControl.VisualAlert=VisualAlert.none):
    super().__init__("Openpilot 暫時失效", alert_text_2, AlertStatus.normal,
...
def startup_master_alert(CP: car.CarParams, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  ...
  return StartupAlert("即將啟動，請謹慎駕駛", branch, alert_status=AlertStatus.userPrompt)
...
def calibration_incomplete_alert(CP: car.CarParams, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  return Alert(
    "路線校正中: %d%%" % sm['liveCalibration'].calPerc,
...
def no_gps_alert(CP: car.CarParams, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  ...
  return Alert(
    "GPS 訊號不良",
    "如果天氣佳，則可能是硬件出現故障" if gps_integrated else "檢查 GPS 天線位置",
    AlertStatus.normal, AlertSize.mid,
...
EVENTS: Dict[int, Dict[str, Union[Alert, AlertCallbackType]]] = {
  ...
  EventName.cameraMalfunction: {
    ET.PERMANENT: NormalPermanentAlert("相機故障", "可能有硬體問題"),
  },
  ...
  EventName.sensorDataInvalid: {
    ...
    ET.NO_ENTRY: NoEntryAlert("沒有來自設備感應器的數據"),
  },
  ...
  EventName.calibrationInvalid: {
    ET.PERMANENT: NormalPermanentAlert("校準無效", "請調整設備位置並重新校準"),
  ...
  EventName.calibrationIncomplete: {
    ...
    ET.SOFT_DISABLE: soft_disable_alert("正在進行校準"),
  ...
  EventName.espDisabled: {
    ET.SOFT_DISABLE: soft_disable_alert("ESP 關閉"),
    ET.NO_ENTRY: NoEntryAlert("ESP 關閉"),
  },
  ...
  EventName.processNotRunning: {
    ET.NO_ENTRY: NoEntryAlert("系統故障: 請重新啟動您的設備"),
  },
