...
class LongitudinalMpc:
  ...
  def set_weights(self, prev_accel_constraint=True, v_lead0=0, v_lead1=0):
    TFs = [0.8, 1.45, 1.8]
    jerk_factor_tf = interp(self.t_follow, TFs, [.35, .5, 1.])
    v_ego = self.x0[1]
    v_ego_bps = [0, 10]
    j_ego_v_ego = 1
    a_change_v_ego = 1
    ...
    if self.mode == 'acc':
      ...
      cost_weights = [X_EGO_OBSTACLE_COST, X_EGO_COST, V_EGO_COST, A_EGO_COST, jerk_factor_tf * a_change_cost * a_change_v_ego, jerk_factor_tf * J_EGO_COST * j_ego_v_ego]
  ...
  def update_TF(self, carstate, personality=log.LongitudinalPersonality.standard):
    ...
    if self.adjustable_follow or self.adjustable_follow_car or personality is not None:
      if self.adjustable_follow_car:
        profile_key = carstate.distanceLines
      elif self.adjustable_follow:
