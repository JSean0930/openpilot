...
def get_T_FOLLOW(v_ego, personality=log.LongitudinalPersonality.standard):
  ...
  if personality == log.LongitudinalPersonality.relaxed:
    ...
    base = 0.7 + 0.0017 * v_kph #1.0 + 0.0024 * v_kph 
  elif personality == log.LongitudinalPersonality.aggressive:
...
class LongitudinalMpc:
  ...
  @staticmethod
  # 🌟 優化 1：修改簽名，允許傳入淨化後的距離 (override_d_rel)
  ...
  # 🌟 優化 2：攔截原始距離
  def process_lead(self, lead, override_d_rel=None):
    v_ego = self.x0[1]
    if lead is not None and lead.status:
      # 如果有傳入平滑過濾的距離，就用它；否則退回原始雷達距離
      x_lead = override_d_rel if override_d_rel is not None else lead.dRel
      v_lead = lead.vLead
      a_lead = lead.aLeadK
  ...
  def update(self, radarstate, v_cruise, x, v, a, j,
    ...
    # ==========================================================
    # 🌟 核心革新 3：在 MPC 內部實作距離互補濾波器 (Complementary Filter)
    # 把這道防線建在 MPC 入口處，徹底淨化 ACADOS 的非線性運算成本
    # ==========================================================
    lead_one = radarstate.leadOne
    filtered_d_rel_0 = None
    
    if lead_one is not None and lead_one.status:
      closing = v_ego - lead_one.vLead
      
      if not self.has_lead_prev:
        self.filtered_d_rel = lead_one.dRel
      else:
        # 物理推算：新距離 = 舊距離 - (我們比前車快的速度 * 經過的時間)
        estimated_d_rel = self.filtered_d_rel - (closing * self.dt)
        # 90% 信任速度推算的平滑軌跡，10% 信任雷達的跳動錨點
        self.filtered_d_rel = 0.90 * estimated_d_rel + 0.10 * lead_one.dRel
        
      self.has_lead_prev = True
      filtered_d_rel_0 = self.filtered_d_rel
    else:
      self.has_lead_prev = False
      filtered_d_rel_0 = None
      self.filtered_d_rel = 0.0
    ...
    # 🌟 將淨化後的距離傳入處理函數
    lead_xv_0 = self.process_lead(radarstate.leadOne, override_d_rel=filtered_d_rel_0)
    lead_xv_1 = self.process_lead(radarstate.leadTwo) # Lead2 影響不大，保留原生資料即可
    ...
    if self.mode == 'acc':
      ...
      # ⚠️ 註：由於 Planner 已經強制鎖死 mode = 'acc'，以下這段原本供低速蠕行的複雜代碼其實不會被觸發了
      # 但為保證程式碼完整性，我們原封不動保留您的原始邏輯
