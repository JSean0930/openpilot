...
CRASH_DISTANCE = .25
LEAD_DANGER_FACTOR = 0.73 #0.75/0.525 /0.368
LIMIT_COST = 1e6
...
COMFORT_BRAKE = 1.1 #2.5/1.5/1.35/1.25
# STOP_DISTANCE = 6.0
...
def get_dynamic_follow(v_ego, personality=log.LongitudinalPersonality.standard):
  ...
  if personality==log.LongitudinalPersonality.relaxed:
    x_vel =  [0.0,    9.7,     14,      28,       35]     #[0.0,  5.0,   13.90,  20,    25,    40]
    y_dist = [0.9,    1.05,    1.15,    1.35,     1.4]    #[1.2,  1.2,   1.5,   1.8,   2.2,  2.2]
  elif personality==log.LongitudinalPersonality.standard:
    x_vel =  [0.0,    9.7,     14,      28,       35]     #[0.0,  5.0,   13.90,  20,    25,    40]
    y_dist = [0.9,    1.05,    1.15,    1.35,     1.4]    #[1.1,  1.1,   1.3,    1.45,  1.6,  1.6]
  elif personality==log.LongitudinalPersonality.aggressive:
    x_vel =  [0.0,    9.7,     14,      28,       35]     #[0.0,  5.0,   12.00,  15.,   20,    25,    40]
    y_dist = [0.9,    1.05,    1.15,    1.35,     1.4]    #[1.05, 1.10,  1.20,   1.20,  1.25,  1.25,   1.3]
  else:
...
def get_stopped_equivalence_factor(v_lead, v_ego):
  ...
  v_diff_offset = 0
  v_diff_offset_max = 12 #12
  speed_to_reach_max_v_diff_offset = 24 #26 # in kp/h
...
class LongitudinalMpc:
  ...
  def update(self, radarstate, v_cruise, x, v, a, j, personality=log.LongitudinalPersonality.standard, dynamic_follow=False):
    ...
    self.smoother_braking = True if self.mode == 'acc' and np.any(v_ego < 5) and np.any(lead_xv_0[:,0] < 40) and not np.any(lead.dRel < (v_ego - 1) * t_follow) else False
