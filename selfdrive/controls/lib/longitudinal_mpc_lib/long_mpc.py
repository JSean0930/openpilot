...
def gen_long_model():
  ...
  model.p = vertcat(a_min, a_max, x_obstacle, prev_a, lead_t_follow, desired_stop_distance, lead_danger_factor)
...
def gen_long_ocp():
  ...
  desired_stop_distance = ocp.model.p[6]

  ocp.cost.yref = np.zeros((COST_DIM, ))
  ocp.cost.yref_e = np.zeros((COST_E_DIM, ))

  desired_dist_comfort = get_safe_obstacle_distance(v_ego, lead_t_follow, desired_stop_distance)
...
class LongitudinalMpc:
  ...
  def update_TF(self, carstate):
    self.adjustable_follow_car = carstate.adjustableFollowCar
    if self.adjustable_follow or self.adjustable_follow_car:
      if self.adjustable_follow_car:
        profile_key = carstate.distanceLines
      else:
        profile_key = int(Params().get("AdjustableFollowDistanceProfile"))
  ...
  def update(self, carstate, radarstate, v_cruise, x, v, a, j, prev_accel_constraint):
    ...
    self.params[:,6] = self.desired_stop_distance
