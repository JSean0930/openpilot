...
class LongitudinalPlanner:
  ...
  def update(self, sm, dp_flags=0):
    ...
    # - AEM 可能改 mode
    #
    # 注意：即使 mode 被改成 blended，只要 e2e_only_active=True，
    #       我們仍會走「低速純E2E輸出」分支，不受 blended 影響。
    # ------------------------------------------------------------
    ...
    # [可選阻擋] 低速純E2E時，避免 AEM 把模式切來切去（通常不擋也沒差，因為輸出分支會覆蓋）
    ...
    # - 轉彎加速限制：維持只在 ACC 套用（避免低速E2E在彎中被過度壓加速）
    # ============================================================
    ...
    # 解析 model 縱向軌跡（給 MPC 用，也可給低速純E2E用）
    ...
    # ============================================================
    # 方法B：明確把 a_min/a_max 傳進 long_mpc（ACC + blended 皆生效）
    # - 方案5B：MPC「照跑」當 safety monitor（FCW、解的合理性）
    # - 但若 e2e_only_active & BLOCK_DTSC=True：就不套用 DTSC 產生的 per-stage 限制
    #   （避免低速純E2E還被曲率模組縮手縮腳）
    # ============================================================
    ...
    # E2E 軌跡：用 model 的 v/a（已經是 T_IDXS_MPC 長度）
    ...
    # ====== FCW：沿用 MPC crash_cnt（方案5B核心）=====
    self.fcw = self.mpc.crash_cnt > 2 and not sm['carState'].standstill
    if self.fcw:
      cloudlog.info("FCW triggered")

    # ====== 決定：低速純E2E or 原本模式（acc/blended）=====
    # 低速純E2E：輸出使用 modelV2.action，但 accel_clip、slew、throttle gating 仍會保護
    a_e2e_action, should_stop_e2e = _safe_model_action(sm)

    # MPC action（用你原本的 get_accel_from_plan）
    ...
    if self.e2e_only_active:
      # ==========================================================
      # 低速純E2E（方案5B）
      # - MPC仍照跑：提供 crash_cnt/FCW/監控
      # - 最終輸出 aTarget/shouldStop：完全使用 model action
      # - publish trajectory：使用 model 的 v/a（以及差分 jerk）
      # - 若你把 E2E_ONLY_BLOCK_ACM=True：ACM 不會改到低速純E2E
      # ==========================================================
      self.v_desired_trajectory = v_traj_e2e
      self.a_desired_trajectory = a_traj_e2e
      self.j_desired_trajectory = j_traj_e2e

      output_a_target = a_e2e_action
      self.output_should_stop = bool(should_stop_e2e)
    else:
      # ==========================================================
      # 原本邏輯（acc:用MPC / blended:min(mpc, e2e)）
      # publish trajectory：以 MPC 為主（與你原版一致）
      # ==========================================================
    ...
    # - 若你開啟 E2E_ONLY_BLOCK_ACM=True：低速純E2E不會進到這裡
    if acm_enabled:
