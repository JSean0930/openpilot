...
def get_lead(v_ego: float, ready: bool, tracks: dict[int, Track], lead_msg: capnp._DynamicStructReader,
  ...
  if vision_active:
    # 這裡實作 Optimization 1: 將原本 Track 裡的動態 aLeadTau 邏輯搬進純視覺
    base_tau = float(np.interp(v_ego, LEAD_ACCEL_TAU_V_EGO_BP, LEAD_ACCEL_TAU_V_EGO_V))
    ...
    vision_dict = {
      ...
      "vRel": float(vision_state.v_rel_filter.x),            # 使用濾波後數值
      "vLead": float(v_ego + vision_state.v_rel_filter.x),   # 使用濾波後數值
      "vLeadK": float(v_ego + vision_state.v_rel_filter.x),
      "aLeadK": float(vision_state.a_lead_filter.x),         # 使用濾波後數值
      "aLeadTau": float(vision_tau),                         # 動態 Tau 參數
      "fcw": False,
