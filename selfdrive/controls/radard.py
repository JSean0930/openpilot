...
def get_lead(v_ego: float, ready: bool, tracks: dict[int, Track], lead_msg: capnp._DynamicStructReader,
  ...
  if track is not None:
    ...
    # 【修改重點 1】真正的 Sensor Fusion：空間位置強制交回給視覺
    lead_dict['yRel'] = float(-lead_msg.y[0])
    lead_dict['modelProb'] = float(lead_prob)
    
    # 【修改重點 2】動態前車加速度延遲 (Dynamic aLeadTau)
    # 解決低速起步遲疑，高速保有平順度
    ...
    if abs(track.aLeadK) < 0.5:
      lead_dict["aLeadTau"] = base_tau
    else:
      lead_dict["aLeadTau"] = 0.0
  ...
  def update(self, sm: messaging.SubMaster, rr: car.RadarData):
    ...
    if len(leads_v3) > 1:
      for i in range(2):
        ...
        y_rel_abs = abs(leads_v3[i].y[0])
        
        # 【修改重點 3】Cut-out 果斷加速邏輯
        # 如果機率上升，或者前車橫向偏移超過 1.2m (正在切出車道)，強制重置濾波器，打破幽靈煞車
        if lead_prob > self.lead_prob_filters[i].x or y_rel_abs > 1.2:
