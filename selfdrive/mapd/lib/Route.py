  ...
  def __init__(self, current, wr_index, way_collection_id, query_center):
    ...
    while True:
      ...
      if len(way_relations) == 1:
        ...
        # If it could not be split, we are done.
        if len(new_wrs) != 2:
          break

        # Replace the original way relation for the split version on way_relations and track split wrs.
